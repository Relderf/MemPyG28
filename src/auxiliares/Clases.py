...
class Timer():
    ...
    def get_tiempo_restante(self):
        ...
        (no dividido en minutos y segundos)
        que quedan de juego """
        ahora = self.get_tiempo_actual()
        max = self.get_tiempo_max()
    ...
    def tiempo_restante(self):
        ...
        return self._tiempo_max - ((self._tiempo_actual//100)%60)
...
class Partida(Timer):
    ...
    def progreso_partida(self, y, x):
        ...
        if len(self._encontradas) == self._grupos:
            puntos_extra = int(self.get_tiempo_restante())
